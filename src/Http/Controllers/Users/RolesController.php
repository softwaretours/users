...
class RolesController extends Controller {
	...
	public function index() {
		...
		return view( 'auth.users.roles.index', [
			'roles'  => $this->rolesInterface->buildViewEditData(),
	...
	public function changePermission( Request $request ) {
		...
		$has_permission = $request->get( 'has_permission' );

		if ( $has_permission == 1 ) {
			$this->rolesInterface->addRolePermission( $role_id, $permission_id );
		} else {
			$this->rolesInterface->deleteRolePermission( $role_id, $permission_id );
		}
